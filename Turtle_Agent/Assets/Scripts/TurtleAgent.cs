...
using Unity.MLAgents.Actuators; // classes related to agents actions   
using Unity.MLAgents.Sensors; // allows agents to collect observations from the enviroment
...
public class TurtleAgent : Agent
{
    ...
    private Renderer _renderer; // stores a reference to the turtles renderer variable
    ...
    public override void CollectObservations(VectorSensor sensor) // agents preceives its enviroment
    {
        // The Goal's position
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            // Apply a small negtive reward when collision starts
            AddReward(-0.05f);
    ...
    private void OnCollisionStay(Collision collision)
    {
        if (collision.gameObject.CompareTag("Wall"))
        { 
            // Continually penalize the agent while it in contact with the wall
